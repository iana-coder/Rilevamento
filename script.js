...
// Mostra storico rilevamenti (dal più recente)
function renderLog() {
  logEl.innerHTML = "";
...
// Mostra dati ricevuti e campi manuali (dal più recente)
function renderDataList() {
  dataList.innerHTML = "";
  [...log].reverse().forEach((e, i) => {
    ...
    altInput.oninput = (ev) => {
      log[log.length - 1 - i].altitude = ev.target.value;
      localStorage.setItem("bussolaLog", JSON.stringify(log));
    };
    ...
    distInput.oninput = (ev) => {
      log[log.length - 1 - i].distance = ev.target.value;
      localStorage.setItem("bussolaLog", JSON.stringify(log));
    };
...
// Chiudi sessione e archivia
function chiudiSessione() {
  const sessioni = JSON.parse(localStorage.getItem("sessioniMajella") || "[]");
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  sessioni.push({ nome: `Sessione-${timestamp}`, dati: log });
  localStorage.setItem("sessioniMajella", JSON.stringify(sessioni));

  log = [];
  localStorage.setItem("bussolaLog", JSON.stringify(log));
  renderLog();
  renderDataList();
  alert("Sessione chiusa e archiviata.");
}
...
function renderArchivio() {
  ...
  if (sessioni.length === 0) {
    archivioList.innerHTML = "<p>Nessuna sessione archiviata.</p>";
    return;
  }

  sessioni.reverse().forEach((sessione, i) => {
    const wrapper = document.createElement("div");
    wrapper.style.marginBottom = "15px";

    const title = document.createElement("h3");
    title.textContent = sessione.nome;

    const list = document.createElement("ul");
    sessione.dati.forEach(entry => {
      const li = document.createElement("li");
      li.textContent = `${entry.time} → ${entry.dir} (${entry.deg}°) Alt: ${entry.altitude || "...."} Dist: ${entry.distance || "...."}`;
      list.appendChild(li);
    });

    wrapper.appendChild(title);
    wrapper.appendChild(list);
    archivioList.appendChild(wrapper);
  });
}
